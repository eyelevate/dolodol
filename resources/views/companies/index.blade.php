...
<div class="container-fluid">
	...
	<bootstrap-card use-header="true" use-body="true" use-footer="true">
		<template slot="header">Company Search Results</template>
		<template slot="body">
...
@if (count($rows) > 0)
	@foreach($rows as $row)
		<bootstrap-modal id="viewModal-{{ $row->id }}" b-size="modal-lg">
			<template slot="header">View Company - {{ $row->name }}</template>
			<template slot="body">
				<!-- Name -->
				<bootstrap-readonly
					use-input="true"
			...
			<template slot="footer">
				...
				<a href="{{ route('company.edit',$row->id) }}" class="btn btn-primary">Edit</a>
			</template>
		</bootstrap-modal>
		{!! Form::open(['method'=>'delete','route'=>['company.destroy',$row->id]]) !!}
